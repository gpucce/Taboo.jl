...
# ╔═╡ afc53b7c-1622-11ec-20d6-8fd4841cfd6d
using PlutoUI, Luxor, JSON,Colors

# ╔═╡ 964ff57e-ed45-4c09-968d-933cd082671a
@bind go CheckBox()

# ╔═╡ fd3bf643-c8dc-422a-8570-1f2208dfe85e
@bind keyword TextField()

# ╔═╡ 7b0054be-eba0-450b-a441-98046ff52ac8
if go
	run(`python test_spikex.py`)
	placeholder = 2
end

# ╔═╡ 2896a04f-b0eb-4468-a3f8-0d29e20676a3
begin
placeholder
oldcards2be = JSON.Parser.parse(open("Category:$(keyword).json"))
cards2be = Dict()
for i in keys(oldcards2be)
	newkey = replace(i, "_"=>" ")
	cards2be[newkey] = filter(replace.(oldcards2be[i], "_"=>" ")) do w
		w != newkey
	end
end
end

# ╔═╡ cdf1f7d1-480d-47dc-9f14-486e114cff02
begin
	if go
	try
		mkdir(keyword)
	...
	pngs = []
	for card2be in cards2be
	word, notwords = card2be
	card = Drawing(600, 600, joinpath(keyword, "$(word).png"))
		origin()
		scale(1.4)
		sethue(RGB(0.8, 0.36, 0.36))
		h_radius = 100
		v_radius = 120
		squircle_width = 2 * h_radius
		squircle_height = 2 * v_radius
		squircle(O, h_radius, v_radius, rt=0.3, :fill)
		...
		sethue("black")
		title = text(word, p_up, halign=:center, valign=:middle)
		# textextents(title) > squircle_width
...
# ╔═╡ deff6f19-db03-4e87-a6c8-3a2bac97a5cf
@bind id NumberField(1:length(pngs))

# ╔═╡ 0868d669-4cf2-4ee3-bc18-f62517f8fcb6
pngs[id]

# ╔═╡ 13fbcb0b-f511-4467-84fc-f9b2a5972904
